...
class ResidenteService extends BaseService<Residente> {
  ...
  async getBySedeRotacion(sedeRotacionId: string): Promise<Residente[]> {
    try {
      return this.getAll([
        where("sedeRotacionId", "==", sedeRotacionId),
        orderBy("nombre"),
      ]);
    } catch (error) {
  ...
  async getResidenteConNotas(
    residenteId: string
  ): Promise<ResidenteConNotas | null> {
    try {
  ...
  async checkDuplicates(
    ...
    try {
      ...
      if (
        !emailSnapshot.empty &&
        (excludeId === undefined || emailSnapshot.docs[0].id !== excludeId)
      ) {
      ...
      if (
        !cuiSnapshot.empty &&
        (excludeId === undefined || cuiSnapshot.docs[0].id !== excludeId)
      ) {
      ...
      if (
        !dniSnapshot.empty &&
        (excludeId === undefined || dniSnapshot.docs[0].id !== excludeId)
      ) {
